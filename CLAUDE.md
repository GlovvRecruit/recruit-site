...
- `app/layout.tsx`에 전역 기본 메타데이터(`metadataBase`, OG/Twitter 카드, keywords), 각 페이지(`about`/`careers`/`brand-jobs`/`media`)에 페이지별 title/description을 넣어놨다. `/onboarding`은 `app/onboarding/layout.tsx`에서 `noindex`(리드 캡처 폼이라 검색 노출 불필요).
- `/careers/[id]`, `/jobs/[id]`는 `generateMetadata` + `JobPosting` JSON-LD(schema.org) 구조화 데이터를 넣어 구글 Job 검색(Google for Jobs) 노출을 노린다. `Job`/`CareersJob` 타입에 `createdAt`을 추가해 `datePosted`로 사용.
- `app/sitemap.ts`(모든 정적 페이지 + 열린 공고 전부), `app/robots.ts`(`/admin`, `/api/` 제외 전체 허용) 자동 생성.
- **네이버 서치어드바이저 등록은 미완료** — https://searchadvisor.naver.com 에서 사용자가 직접 사이트 소유 확인을 받아야 함(코드 발급은 인터랙티브 웹 가입이 필요해 CLI로 대신할 수 없음). 확인 코드를 받으면 `app/layout.tsx`의 `metadata.verification.other["naver-site-verification"]` 주석을 해제하고 코드를 넣을 것.
