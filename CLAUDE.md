...
## 3. 외부 연동 저장소 (직접 코드는 없지만 통합 대상)

- **GlovvRecruit/exam** (로컬: `C:\Users\park7\exam`, 배포: `glovvrecruit.github.io/exam/`) — 인턴 시험
  웹앱. 순수 HTML/JS + **별도 Supabase 프로젝트**. 이 저장소의 admin 인턴 탭이 `exam_attempts` 테이블을
  **읽기 전용**으로 조회해 점수를 보여준다 (`lib/supabase/exam-client.ts`, `NEXT_PUBLIC_EXAM_SUPABASE_URL`/
  `NEXT_PUBLIC_EXAM_SUPABASE_ANON_KEY` 필요, 값은 exam 저장소의 `config.js`에서 그대로 복사).
  점수 매칭은 **이름 문자열 일치**로 하므로, admin에서 인턴을 등록할 때 exam 응시자명과 정확히 같은
  이름을 입력해야 한다.
- **GlovvRecruit/Flixx-CS-chat** ("brand-helper", 로컬: `C:\Users\park7\brand-helper`, 배포:
  `brand-helper.vercel.app`) — 인턴용 사내 Q&A RAG 챗봇(별개 Next.js + Supabase pgvector 프로젝트).
  이 저장소의 admin에는 **링크만** 걸려 있다(깊은 데이터 연동 없음 — 성격이 다른 도구이기 때문).

## 4. 기술 스택

- Next.js 16(App Router) + TypeScript + Tailwind v4. **Vercel 배포**.
- 이 사이트 전용 Supabase 프로젝트 1개: `brands`/`jobs`/`leads`/`careers_jobs`/`career_applications`/
  `media_links`/`interns` (`supabase/migrations/0001_init.sql`).
- 관리자 인증: **Supabase Auth 이메일/비밀번호** (exam 프로젝트와 동일한 패턴). 관리자 계정은
  Supabase Dashboard → Authentication → Users에서 수동 생성. GitHub OAuth 아님.
...
- [ ] 실제 Supabase 프로젝트 생성 + `0001_init.sql` 적용 + 관리자 계정 생성 (사용자가 직접)
- [ ] TOP 브랜드 실제 데이터 입력 (admin 채용 공고 탭에서)
- [ ] 카카오 친구톡 발송 파이프라인(pg_cron + 발송대행사 API) — 현재는 리드 캡처까지만 구현, 실제
  발송은 미구현. 원래 PROJECT_SPEC의 "신규 감지 → 다이제스트 발송" 로직은 이번 리디자인에서 범위 밖.
- [ ] Tally 지원서 폼 ID를 실제 값으로 교체 (`components/CareersApplyModal.tsx`의 TODO 주석 참고)
- [ ] GitHub(GlovvRecruit 조직)에 저장소 푸시 + Vercel 프로젝트 연결 + 환경변수 설정
